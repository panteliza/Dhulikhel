...
const RoomTypesDisplay = () => {
  return (
    <motion.div
      className="px-6 py-12 max-w-screen-xl mx-auto bg-gradient-to-b from-white to-[#f6f9ff]"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {roomTypes.map((room, index) => (
          <motion.div
            ...
            initial="hidden"
            animate="visible"
            className="border border-gray-100 bg-white rounded-3xl shadow-lg p-6 hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 hover:scale-105 backdrop-blur-sm group relative overflow-hidden"
          >
            <motion.div
              className="absolute inset-0 bg-gradient-to-br from-[#e2f0ff] to-transparent opacity-0 group-hover:opacity-60 transition duration-700 rounded-3xl z-0"
              initial={{ opacity: 0 }}
              animate={{ opacity: 0.3 }}
            />
